...
file = pd.read_excel("Data_svd.xlsx")
...
###___________U là vecto kỳ dị trái
##_____________S là ma trận kỳ dị
#______________V^T là ma trận chuyển vị của vecto kỳ dị phải


###______________________________Xấp xỉ A qua 3 giá trị kỳ dị lớn nhất_________________________###

Appro_A = np.zeros((m,n))
for idx in range(3):
    U1x = (U.T)[idx]
    U1 = np.zeros((m,1))
    for j in range(m):
        U1[j][0] = U1x[j]
    VT1 = np.array([VT[idx]])
    Appro_A += np.dot(np.dot(U1,S[idx][idx]),VT1)
print('')
print('Xấp xỉ A =')
print(Appro_A)
